...
**Loop Engineering（循环工程）落地脚手架：把一句模糊的「我想自动搞定 X」，锻造成二元可验证的 `goal`，并备齐一整套能自跑的 Loop 设定，最后把可一键执行的命令交到你手上——扳机你自己扣。**
...
**愚公**把这些坑都焊进流程。它不替你跑 loop，而是把你的愿望**备料到「一键可发」**：goal 锻造成机器能判真假的判据，maker/checker 分工、worktree 隔离、心跳触发、状态持久化全部架好，命令渲染成你那个 runtime（Claude Code 或 Codex）能直接跑的样子。最后那一下扳机，永远是你的祈使句。
...
愚公走完六步（采石 → 立志 → 分工 → 通路 → 刻石 → 交令），交付一段可直接复制的 `/goal`：
...
```text
帮我把「<你想反复自动化的事>」做成一个 Loop Engineering 循环，出 goal 和双 runtime 命令。
```
...
| 架脚手架 | maker/checker prompt、worktree 隔离、连接器、心跳触发 |
...
│  ├─ templates/                     # REPOS / manifest / maker / checker / feedback
│  └─ examples/
...
合格表现：愚公会拒绝模糊判据、逼你把「triage 完」定义成机器可验证的条件，产出带 `done_when`、护栏、双 runtime 命令的完整设计，并在最后停手等你开 goal。
